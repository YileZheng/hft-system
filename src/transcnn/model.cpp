...
#pragma HLS UNROLL factor=COUT
        pricebase_t tmp0[INPUT_LENGTH][INPUT_SIZE], tmp1[INPUT_LENGTH][INPUT_SIZE], tmp2[INPUT_LENGTH][INPUT_SIZE];
        multihead_attn(tmp0, price, teattninw[ci], teattninb[ci], teattnoutw[ci], teattnoutb[ci]);
    ...
    // decoder
    pricebase_t ctmp0pad[COUT][INPUT_LENGTH+OUTPUT_LENGTH][1], ctmp1[COUT][INPUT_LENGTH-KERNEL_SIZE+1+OUTPUT_LENGTH];
    pricebase_t ctmp0phy[COUT][INPUT_LENGTH];
    pricebase_t out_pred[1][INPUT_LENGTH-KERNEL_SIZE+1+OUTPUT_LENGTH];


    for (int pi = 0; pi < COUT; pi++){
        ctmp0phy[pi] = ctmp0pad[pi];
    }

    conv2d<COUT, INPUT_SIZE, INPUT_LENGTH, COUT, 1, 1, INPUT_SIZE>(ctmp0phy, outChannels, compressw, compressb);
    ...
    for (int io = 0; io < OUTPUT_LENGTH; io++){
        prediction[io] = out_pred[INPUT_LENGTH-KERNEL_SIZE+io];
    }
...
#pragma HLS DATAFLOW
    ...
    static scaling = powf(HEAD_DIM, -0.5);
...
#pragma HLS UNROLL factor=NUM_HEAD
...
#pragma HLS UNROLL factor=LENGTHWISE_UNROLL
    ...
    // QK softmax
    softmax<INPUT_LENGTH, INPUT_LENGTH>(qks, qk);
...
#pragma HLS UNROLL factor=HEAD_DIM
                pricebase_t v1 = 0;
